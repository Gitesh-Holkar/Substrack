// src/pages/LandingPage.tsx - EXACT REPLICA OF HTML
  ...
  useEffect(() => {
    ...
    if (window.matchMedia("(pointer: fine)").matches) {
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        if (cursorTooltip) cursorTooltip.style.transform = `translate(-50%, -250%) translate(${dotX}px, ${dotY}px)`;
      ...
      const followRing = () => {
        ...
        if (cursorRing) cursorRing.style.transform = `translate(-50%, -50%) translate(${ringX}px, ${ringY}px)`;
        requestAnimationFrame(followRing);
      };
      requestAnimationFrame(followRing);

      const hoverElements = document.querySelectorAll('a, button, .tab-button, .feature-card, .before-after-card');
      ...
      if (logo) {
        logo.addEventListener('mouseenter', () => document.body.classList.add('tooltip-visible'));
        logo.addEventListener('mouseleave', () => document.body.classList.remove('tooltip-visible'));
      }
